    ...
    const fetchUserSubscription = async () => {
      try {
        ...
        const subscriptions = Array.isArray(customerData.subscriptions) 
          ...
          : customerData.subscriptions ? [customerData.subscriptions] : [];
        const activeSubscription = subscriptions.find(sub => 
          sub && ACTIVE_STATUSES.includes(sub.status as string)
        );

        // 检查是否为Premium订阅
        if (activeSubscription) {
          const isPremiumTier = SUBSCRIPTION_TIERS.some(tier => 
            tier.productId === activeSubscription.creem_product_id && 
            tier.name === "Premium"
          );
          // 兼容直接产品ID检查
          const isPremiumProduct = activeSubscription.creem_product_id === "prod_5Kqq7BxbLyxmO1sM0Pc5LY";
          setIsPremium(isPremiumTier || isPremiumProduct);
        }
      } catch (error) {
        console.error("获取用户数据失败:", error);
        toast({ title: "系统错误", description: "加载数据时发生错误，请稍后重试" });
      } finally {
        setIsLoading(false);
      }
    };
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      if (data.success && data.avatarUrl) {
        ...
        toast({ title: "生成失败", description: data.error || "无法生成头像，请重试" });
      }
    } catch (error) {
